...
void setup()
{
  ...
  if ( !SD.begin() )
    ...
    if( SD.exists("datalog.csv") == 0 )   // test if file with name 'Log.txt' already exists
    {
      Serial.println("\r\nCreate 'datalog.csv' file ... ");
      dataLog = SD.open("datalog.csv", FILE_WRITE);   // create (&open) file Log.txt
      if(dataLog) {                               // if the file opened okay, write to it:
        Serial.println("OK");
        
        //? write some texts to 'datalog.txt' file
        dataLog.println("DATETIME, TEMP, WATER DISTANCE");
        dataLog.close();
  ...
  Serial.println("\r\n    DATE    |    TIME  | TEMPERATURE | WATER DISTANCE");
  Serial.println("(dd-mm-yyyy)|(hh:mm:ss)|");
}

int iterationChecker = 0;
...
void loop()
{
  ...
  //? Drop vitamin
  if(distance<=10 && iterationChecker == 0){
  ...
  if( (now.second() % 10 == 0) && (p_second != now.second()) )
  {
    ...
    // Serial.print(buffer2); // datetime
    // Serial.println(buffer1); // temp

    // write data to SD card
    if(sd_ok)
    {
      char str[80];
      sprintf(str, "%02u-%02u-%04u | %02u:%02u:%02u, %s, %d",now.day(), now.month(), now.year(),
                      now.hour(), now.minute(), now.second(), buffer1, distance);

      Serial.println(str);

      dataLog = SD.open("datalog.csv", FILE_WRITE);
      //TODO: write to datalog.csv
      dataLog.println(str);
...
//? function for button1 (B1) debounce
bool debounce ()
{
  byte count = 0;
  for(byte i = 0; i < 5; i++)
